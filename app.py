...
def format_datetime(dt_string):
    ...
    try:
        if dt_string:
            # SQLite datetime formatından parse et
...
def get_all_products():
    ...
    for product in products:
        ...
        if product_dict['last_updated']:
            try:
                dt = datetime.fromisoformat(product_dict['last_updated'])
                product_dict['last_updated_formatted'] = dt.strftime('%d.%m.%Y %H:%M')
            except:
                product_dict['last_updated_formatted'] = product_dict['last_updated']
        else:
            product_dict['last_updated_formatted'] = '-'
...
def add_product():
    ...
    if request.method == 'POST':
        try:
            ...
            error_msg = 'Lütfen geçerli sayısal değerler girin!'
            if is_json_request:
                return jsonify({'success': False, 'message': error_msg})
            else:
                flash(f'❌ {error_msg}', 'error')
